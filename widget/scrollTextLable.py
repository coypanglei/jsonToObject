...
class scrollTextLabel(QLabel):
    def __init__(self, parent=None):
        ...
        self.newX = 10
        self.t = QTimer()
        self.font = QFont(('微软雅黑, verdana'), 8)
        self.t.timeout(self.changeTxtPosition())
    ...
    def paintEvent(self, event):
        ...
        self.rect = painter.drawText(QRect(0, -7, self.width(), 25), Qt.AlignHCenter | Qt.AlignVCenter,
                                     self.txt)

        if self.rect.width() > self.width():
            # 如果绘制文本宽度大于控件显示宽度，准备滚动：
            painter.setPen(QColor(255, 255, 255, 255))
            painter.drawText(QRect(self.newX, -7, self.textRect.width(), 25), Qt.AlignLeft | Qt.AlignVCenter,
                             self.txt)
            # 每150ms毫秒滚动一次
            self.t.start(150)
        else:
